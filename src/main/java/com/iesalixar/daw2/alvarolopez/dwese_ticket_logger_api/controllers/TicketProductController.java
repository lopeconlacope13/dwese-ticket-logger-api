...
import com.iesalixar.daw2.alvarolopez.dwese_ticket_logger_api.repositories.ProductRepository;
import com.iesalixar.daw2.alvarolopez.dwese_ticket_logger_api.repositories.ProvinceRepository;
import com.iesalixar.daw2.alvarolopez.dwese_ticket_logger_api.repositories.TicketRepository;
...
public class TicketProductController {
   ...
   //@Autowired
     ...
     */
    @PostMapping("/insert")
    public String insertTicket(@Valid @ModelAttribute("ticket") Ticket ticket,  BindingResult result,  RedirectAttributes redirectAttributes, Locale  locale, Model model) {
     ...
     */
    @PostMapping("/update")
    public String updateTicket(@Valid @ModelAttribute("ticket") Ticket ticket,  BindingResult result,  RedirectAttributes redirectAttributes, Locale  locale, Model model) {
    ...
    public String addNewProductToTicket(@RequestParam("ticketId") Long  ticketId, @RequestParam("productName") String productName,  @RequestParam("productPrice")  BigDecimal productPrice, RedirectAttributes redirectAttributes) {
...
// Verificar si ya existe un producto con el mismo nombre en el ticket
                boolean productExists = ticket.getProducts().stream().anyMatch(product -> product.getName().equalsIgnoreCase(productName));
                if (productExists) {
